# Hi, I'm Mohammed Fannana 👋

### Full Stack Web Developer 

---

Software Engineer and Full Stack Laravel Developer with 4+ years of experience
designing and building scalable web applications, RESTful APIs, and secure backend
systems. 

I specialize in Laravel backend architecture, database design and optimization,
and RESTful API development. I have hands-on experience integrating blockchain-based
verification systems, improving API performance by up to 30%, and reducing system
errors through clean code practices and rigorous code reviews.

I'm passionate about writing efficient, maintainable code and continuously improving
system performance. Whether working independently or leading a team, I bring a
problem-solving mindset and a strong commitment to delivering high-quality software
on time and at scale.


---


## 🛠️ Skills

**Backend:**
![PHP](https://img.shields.io/badge/PHP-777BB4?style=flat&logo=php&logoColor=white)
![Laravel](https://img.shields.io/badge/Laravel-FF2D20?style=flat&logo=laravel&logoColor=white)
![REST API](https://img.shields.io/badge/REST%20API-005571?style=flat&logo=fastapi&logoColor=white)
![Ajax](https://img.shields.io/badge/Ajax-0769AD?style=flat&logo=jquery&logoColor=white)

**Frontend:**
![HTML](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![CSS](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![Vue.js](https://img.shields.io/badge/Vue.js-4FC08D?style=flat&logo=vuedotjs&logoColor=white)
![Bootstrap](https://img.shields.io/badge/Bootstrap-7952B3?style=flat&logo=bootstrap&logoColor=white)

**Database:**
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql&logoColor=white)

**Tools:**
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=flat&logo=postman&logoColor=white)

**Additional:**
![Blockchain](https://img.shields.io/badge/Blockchain-121D33?style=flat&logo=bitcoin&logoColor=white)

---

## 📌 Featured Projects

### 🔹 Ribano Investment Platform
A high-concurrency investment platform with real-time transaction verification.
- Integrated blockchain hashing for tamper-proof financial records
- Designed scalable Laravel APIs with optimized database indexing
> Tech: Laravel, MySQL, REST APIs, Blockchain
> 🔗 [ribano.org](https://ribano.org)

### 🔹 Rushd Academy Educational Platform
A full-featured educational platform for course management and enrollment.
- Built role-based authentication and secure data handling
- Designed REST APIs consumed by dynamic Vue.js frontend
> Tech: Laravel, MySQL, REST APIs, Vue.js
> 🔗 [rushd.academy](https://rushd.academy)

---

## 📬 Contact Me

- 📧 Email: mohamed.fannana10@gmail.com
- 📱 Phone: +972 592 851 883
- 💼 LinkedIn: [linkedin.com/in/mohammed-fannana](https://linkedin.com/in/mohammed-fannana)
- 🐙 GitHub: [github.com/mohammed-fannana](https://github.com/mohammed-fannana)
- 🌍 Location: Palestine

---

⭐ *Feel free to explore my repositories and don't forget to star what you like!*
